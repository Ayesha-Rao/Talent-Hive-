...
const getOpenTasks = async (req, res) => {
    try {
        const tasks = await Task.find({ status: "open" }).populate("clientId", "name email");
...
// ✅ Assign task to a freelancer (Agency Owners only)
// const assignTask = async (req, res) => {
//     try {
//         const { taskId, freelancerId } = req.body;

//         if (req.user.role !== "agencyOwner") {
//             return res.status(403).json({ message: "Only Agency Owners can assign tasks." });
//         }
...
const assignTask = async (req, res) => {
    try {
        ...
        if (req.user.role !== "client" && req.user.role !== "agencyOwner") {
            console.log("❌ Access Denied: Only Clients and Agency Owners can assign tasks.");
            return res.status(403).json({ message: "Only Clients and Agency Owners can assign tasks." });
        }
        ...
        if (req.user.role === "client") {
            ...
            return res.status(200).json({ message: "Task assigned successfully by Client", task });
        }
        ...
        return res.status(403).json({ message: "Agency Owners must assign Subtasks, not main tasks." });
...
const completeTask = async (req, res) => {
    try {
        ...
        if (task.assignedTo.toString() !== req.user.id) {
            return res.status(403).json({ message: "Only assigned freelancer can mark task as complete." });
        }
...
module.exports = { createTask, getOpenTasks, getClientTasks, assignTask, completeTask, getTaskById };
