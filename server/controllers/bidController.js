...
const placeBid = async (req, res) => {
    try {
        ...
        if (req.user.role !== "independentFreelancer" && req.user.role !== "agencyOwner") {
            return res.status(403).json({ message: "Only Freelancers & Agencies can place bids." });
        }
...
const viewBids = async (req, res) => {
    try {
        ...
        if (task.clientId.toString() !== req.user.id) {
            return res.status(403).json({ message: "Only the Client who posted the task can view bids." });
        }

        const bids = await Bid.find({ taskId }).populate("bidderId", "name email role");
...
const acceptBid = async (req, res) => {
    try {
        ...
        if (task.clientId.toString() !== req.user.id) {
            return res.status(403).json({ message: "Only the Client who posted the task can accept bids." });
        }
        ...
        await Bid.updateMany({ taskId: bid.taskId, _id: { $ne: bid._id } }, { status: "rejected" });

        console.log("✅ Bid Accepted, Task Assigned:", task);

        res.status(200).json({ message: "Bid accepted successfully, Task assigned", task });
    } catch (error) {
